...
--  @props:
--      permissionsByModule  Collection  ->  gruppo modulo => Collection permessi
--      moduleLabels         array       ->  chiave modulo => label ITA
-- ========================================================================= --}}
@props(['permissionsByModule', 'moduleLabels'])

<div @click.away="showModal = false"
     class="relative bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-y-auto max-h-[90vh] w-full p-6 z-10">
    ...
    <form x-bind:action="mode === 'create'
        ...
        <div class="space-y-6">
            ...
            <div>
                <label for="name"
                       class="block text-xs font-medium text-gray-700 dark:text-gray-300">Nome Ruolo</label>
                    ...
                    @foreach($permissionsByModule as $moduleKey => $perms)
                        <fieldset class="border rounded p-2">
                            <legend class="text-[0.65rem] uppercase font-semibold px-1">
                                {{ $moduleLabels[$moduleKey] ?? \Illuminate\Support\Str::headline(str_replace('.', ' ', $moduleKey)) }}
                            </legend>
                            ...
                            @foreach($perms as $perm)
                                <label class="flex items-center space-x-2 text-xs mt-1">
                                    ...
                                    <span  class="cursor-help"
                                        ...
                                        {{ $perm->display_name ?? Str::headline(Str::afterLast($perm->name,'.')) }}
                                    </span>
                                </label>
                            @endforeach
                        </fieldset>
        ...
        <div class="mt-6 flex justify-end space-x-2">
            <button type="button" @click="showModal = false"
                    class="px-4 py-1.5 text-xs font-medium rounded-md
                           ...
                           hover:bg-gray-300 dark:hover:bg-gray-500">Annulla</button>
