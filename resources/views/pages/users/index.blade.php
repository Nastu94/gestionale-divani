                    ...
                    <tbody class="bg-white dark:bg-gray-800" x-data="{ openId: null }">
                        @foreach($users as $user)
                            <tr @click="openId = openId === {{ $user->id }} ? null : {{ $user->id }}" 
                                :class="openId === {{ $user->id }} ? 'bg-gray-100 dark:bg-gray-700' : ''" 
                            ...
                            <tr x-show="openId === {{ $user->id }}" x-cloak>
                                <td colspan="4" class="px-6 py-2 bg-gray-100 dark:bg-gray-700">
                                    <div class="flex space-x-4 text-xs">
                                        @can('users.update', $user)
                                            <button type="button" @click="openEdit({ id: {{ $user->id }}, name: '{{ addslashes($user->name) }}', email: '{{ $user->email }}', role: '{{ $user->roles->pluck('name')->first() }}' })" 
                                                    class="hover:text-yellow-600">
                                                <i class="fas fa-pencil-alt mr-1"></i>Modifica
                                            </button>
                                        @endcan
                                        @can('users.delete', $user)
                                            <form action="{{ route('users.destroy', $user) }}" method="POST" onsubmit="return confirm('Sei sicuro che vuoi eliminare questo utente?');">
                                                @csrf @method('DELETE')
                                                <button type="submit" class="hover:text-red-600"><i class="fas fa-trash-alt mr-1"></i>Elimina</button>
                                            </form>
                                        @endcan
                                    </div>
                                </td>
